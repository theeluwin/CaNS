...
def calc_batch_rec_metrics_per_k(rankers: LongTensor,
    ...
    for k in ks:
        ...
        # metric_values[f'HR@{k}'] = [float(hr) for hr in hrs.detach().cpu().numpy()]
        ...
        metric_values[f'Recall@{k}'] = [float(recall) for recall in recalls.detach().cpu().numpy()]
        ...
        ndcgs = (dcg / idcg)
        ndcgs[idcg == 0] = 0.0
        metric_values[f'NDCG@{k}'] = [float(ndcg) for ndcg in ndcgs.detach().cpu().numpy()]
